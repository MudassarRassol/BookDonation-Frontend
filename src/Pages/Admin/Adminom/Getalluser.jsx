...
const GetAllUser = () => {
  ...
  const handleSearch = async (name) => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:3000/api/admin/searchuser/${name}`, {
        withCredentials: true,
      });
  ...
  return (
    <div className="p-6 mx-auto h-screen ">
      ...
      <div className='flex items-center gap-2'>
       ...
       <h1 className="text-3xl font-bold text-left ">User List</h1>
       </div>
        <input
          type="search"
          placeholder="Search User"
          className="p-2 rounded-lg outline-none border-b-2"
      ...
      {loading ? (
        ...
        <div className="text-red-500 text-center">
          <img src={nouser} alt="No User" className="w-96 h-96 mx-auto" />
          {error}</div>
      ) : users.length === 0 ? (
        <Nobookmsg img={nouser} Nobookmsg={"No User"} />
      ) : (
